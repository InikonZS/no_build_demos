...
function drawArray3d(arr, idx){
  ctx.lineWidth=3;
  ctx.fillStyle='#f0f';
  ctx.lineJoin='round';
  for (let i=0; i<idx.length/3; i++){
    ctx.beginPath();
    ctx.moveTo(arr[idx[i*3]].x, arr[idx[i*3]].y);
    ctx.lineTo(arr[idx[i*3+1]].x, arr[idx[i*3+1]].y); 
    ctx.lineTo(arr[idx[i*3+2]].x, arr[idx[i*3+2]].y); 
    ctx.lineTo(arr[idx[i*3+0]].x, arr[idx[i*3+0]].y); 
    ctx.stroke(); 
  }
  
}

function makeIndexed(arr, idx){
  let res =[];
  for (let i=0; i<idx.length; i++){
    res.push(arr[idx[i]].x);
    res.push(arr[idx[i]].y);
    res.push(arr[idx[i]].z);
  }
  return res;
}
...
const incx = new Control(mainNode, 'div','','rotate 3d',()=>{
  ang = 0;
  ro = !ro;
})
...
*/
  let la=[new Vector3d(0,0,0), new Vector3d(1000,1000,1000)];
  drawArray2d(la, [0,1]);
  drawArray3d(rectTransformedA, idx3);

  let idxed = makeIndexed(rectTransformedA, idx3);
  let cp = crossMeshByLine(idxed, la[0], la[1]);
  cp.forEach(it=>{
